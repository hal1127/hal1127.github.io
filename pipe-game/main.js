...
class Pipe {
  ...
  rotate() {
    let pi = (parseInt(this.index)+1)%pipes[this.type].length
    pi = pi.toString()
    this.index = pi
    return this
  }
...
class Board {
  ...
  this.pipes = [new Array(5), new Array(5), new Array(5), new Array(5), new Array(5)]
    for (let i = 0; i < 5; i++) {
      for (let j = 0; j < 5; j++) {
        if (bps[i][j] == null) {
          let type = parseInt(Math.random() * 3 + 1).toString()
          let index = parseInt(Math.random() * pipes[type].length).toString()
